...
// THESE ARE ALL THE FUNCTION DEFINITIONS I (AMMAR) PUT IN AS A PART OF THE ASSIGNMENT
void getcwd(char* path, int size);
void getwd(char* path);
void chdir(char* subdir);
void exit(int exitCode);
int atoi(const char* str);
int getopt(int argc, char* const argv[], const char* opstring);
int strcmp(const char* str1, const char* str2);
int h_enter(int deviceNumber, int fileSerialNumber);
...
int	user_file_list_supplied = 0;
...
#ifdef LINUX
	while ((option = getopt_long(argc, argv, "dfh:iostqvVl", long_options, &option_index) /*option = getopt(argc, argv, "dfh:iostqvVl")*/) != EOF) {
#else
	while ((/*option = getopt_long(argc, argv, "dfh:iostqvVl", long_options, &option_index)*/ option = getopt(argc, argv, "dfh:iostqvVl")) != EOF) {
#endif		
		switch (option) {
		...
		if (err) {
			...
			#ifdef LSTAT
				fprintf(stderr,"	-l	do not follow symlinks\n");
			#endif
			exit(-1);
		}
	...
	if (version > 0 ) {
		...
		if (version>1) {
			...
			#endif
			#ifdef LSTAT
				if (sw_follow_links) printf("Do not follow symlinks\n");
			#endif
		}
	}
	...
	if (sw_summary) {
		if (!quick) {	// We shouldn't be displaying the total size if quick is invoked
			printf("\n\nTotal space used: %ld\n",total_sizes);
