...
SYMBOL *new_rule(int v) {
    ...
    //SYMBOL* boolean_pointer = *(rule_map+(v-256));
    SYMBOL* rule_head = new_symbol(v, NULL);
    *(rule_map+(v-256)) = rule_head;
    rule_head->next = rule_head;
    rule_head->prev = rule_head;
    rule_head->rule = rule_head;
    add_rule(rule_head);
    return rule_head;
}
...
void delete_rule(SYMBOL *rule) {
    SYMBOL* last_rule = rule->prevr;
    SYMBOL* next_rule = rule->nextr;
    last_rule->nextr = last_rule->nextr->nextr;
    next_rule->prevr = next_rule->prevr->prevr;
    //rule->nextr = NULL;    Not sure if we should set these to null or delete the symbol struct from symbol storage?
    //rule->prevr = NULL;

    if (main_rule == rule) {
        if (main_rule->next == main_rule) {
            main_rule = NULL;
        } else {
...
void unref_rule(SYMBOL *rule) {
    if (rule) {
        if (rule->refcnt) {
            ...
            fprintf(stderr, "Refcnt cannot be negative");
            abort();
        }
    }
}
